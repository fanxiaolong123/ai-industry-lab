...
**流程（用户拍板，写进 skill）**：每批 2 张卡 → 每页 2 名独立 opus 评审员盲评（任务书薄：指到 page-review skill + spine-card skill + 页面路径）→ 清单并集 → 分歧项＋全部高严重度发现派对抗复核 agent 证伪（错误指控与页面错误同样有害）→ 结果按页面 × 严重度**直接在对话里列给用户，不写报告文件** → 用户确认的清单全部修复。
...
**剩余**：spine08-15（4 批 × 2 卡）。卡评完后手册层 m01-m14 是否评审待用户定。修复后是否直接 push 按用户当次指示（本轮已授权推送过一次）。
