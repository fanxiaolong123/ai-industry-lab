...
- `index.html`：总览主页。四层可展开列表（骨架/案例/手册/实盘），默认收起，不显示进度。品牌 logo 与顶栏「骨架/案例/模块」tab 从此页出发。
- `spine01.html` 至 `spine15.html`：骨架层正式页面，`body[data-page]` 使用 `s01` 至 `s15`。
- `case01.html` 至 `case12.html`：案例层正式页面，`body[data-page]` 使用 `c01` 至 `c12`。当前 `case01.html` 已建（台积电 N3 稀释 2024-01-18 · 决策复盘金标准）。
...
- `CASES` 是 12 个真实决策复盘案例的唯一导航清单，与 `SPINES` 并列。
- `LAYERS` 是 14 个手册模块的唯一导航清单。
- 左侧目录只显示当前层；顶栏「骨架 / 案例 / 模块」三 tab 切换到各层第一个已建页面。主页 `index` 时三 tab 都不高亮。
...
新建或重写 `caseXX.html` 前，先读 `.agents/skills/case-replay/SKILL.md` 和金标准 `case01.html`。每个案例是对一次真实投研决策的忠实重建，七段固定结构：

1. 决策现场（判断日 + 决策方 + 一句 thesis）。
2. 判断点前的信息集（时间线红竖线切割判断日 · 已知数字与事件 · 逐字原话引文 · 市场情绪与共识）。
3. 学习者动作 · 先下判断（第一个动作点 · `<details>` 隐藏专家答案）。
4. 专家推理链五要素：定位 → 机制 → 数量化算账（中途算账题隐藏 · 第二个动作点）→ 反方（≥2 条真对抗）→ 可证伪判断（≥2 条 · 具体到数字与时点）。
5. 反方与失效分析（外部条件变化）。
6. 事后对照与归因（严格与前面切开 · 数字兑现表 · 四象限归因矩阵 · 教训挂回骨架卡）。
7. `.src` 来源清单、完成按钮和前后案例导航。

案例区别于骨架卡/手册页的核心纪律是**时序**：判断点前的信息集必须严格切在判断日之前，发布前 grep 判断日之后的月份/季度/年份，逐条确认不出现在信息集与推理链节。逐字引文必须与原文字面一致，不能是编者转述。
